...
# 一、isinstance(obj,cls)和issubclass(sub,super) （常用）
...
![img](chapter08-Class Advanced.assets/1036857-20161222125118901-555913707.png) 
...
# 七、`__setitem__`,`__getitem__`,`__delitem__` （常用）
...
# 只是改了一下属性操作的形式，把对象的属性模拟成字典的数据结构，进行操作
```



# 八、`__str__`,`__repr__`,`__format__` （常用）
...
当程序结束时，python只会回收自己的内存空间，即用户态内存，而操作系统的资源则没有被回收，这就需要我们定制`__del__`，在对象被删除前向操作系统发起关闭数据库链接的系统调用，回收资源
...
2.在需要管理一些资源比如文件，网络连接和锁的编程环境中，可以在`__exit__`中定制自动释放资源的机制，你无须再去关系这个问题，这将大有用处
...
所有的可调用对象，归根结底执行的都是函数。所有的可调用，到最后都要转换成函数去运行。
