...
```python
group参数未使用，值始终为None。预留的扩展操作。

target表示调用对象，即子进程要执行的任务

args表示调用对象的位置参数元组，args=(1,2,'amy',)

kwargs表示调用对象的字典,kwargs={'name':'amy','age':18}
...
**注意：在windows中Process()必须放到# if __name__ == '__main__':下（Unix中不用）**
...
这是隐藏对Process（）内部调用的原因，使用if __name__ == “__main __”，这个if语句中的代码将不会在导入时被调用。
```
...
p1.start()# 主线程在执行的时候，又开启了一个子进程。完全复制父进程的地址空间，一旦创建成功，就是一个完全独立的空间。创建进程，就是发起系统调用，一旦创建成功，就会在子进程中执行piao()。不立即执行piao()中的打印是因为，创建
p2.start()
p3.start()
p4.start()
print('父进程（主线程）')
# 一执行这个文件，就是运行了一个程序，就会产生一个进程，进程中有一个主线程。
#p1.start()解析
# 主线程在执行的时候，又开启了一个子进程。完全复制父进程的地址空间，一旦创建成功，就是一个完全独立的空间。创建进程，就是发起系统调用，一旦创建成功，就会在子进程中执行piao()的操作。不立即执行piao()中的打印是因为，start()的过程中，拷贝父进程的数据，先创建出子进程，才去执行piao()函数。这个过程发起系统调用，需要时间。我们的程序（主进程）不会在这里等待。
```

```python
父进程先运行完，为什么要等子进程运行完才算执行完成？
子进程属于父进程，如果父进程不存在了，子进程就变成了僵尸进程。
```
...
class Piao(Process):
    def __init__(self,name):
        super().__init__()
        self.name=name
    def run(self): # p.start()就是在调用process类下的run()方法，自己定制进程类，一定要实现这个方法。
...
p1.start() #p1.run()。start会自动调用run。但不能直接调用p1.run()，因为用它启的进程，有些方法不能用。
p2.start()
p3.start()
p4.start()
print('主线程')
...
#而我们p1.join()是在等p1结束,没错，p1只要不结束主线程就会一直卡在原地,这也是问题的关键
...
p1.join() # 主进程在等子进程p1运行完成。这个join是把主进程join住，子进程在后台仍然运行。不会影响整体的速度。
p2.join()
p3.join()
p4.join()
...
**![img](chapter12-Multiprocess.assets/1036857-20180711125930278-2000468053.png)**
...
# 锁：
为了防止大家同时修改同一块数据导致的抢占状况。
# 队列：
先进先出。比如，自动扶梯，先上的人先下。吃了拉，先吃进去的先拉出来。
# 堆栈：
先进后出。比如，把叠好的衣服放到箱子里，后放进去的，先出来。吃了吐，后吃进去的先吐出来。
```
...
*在并发编程中使用生产者和消费者模式能够解决绝大多数并发问题。该模式通过平衡生产进程和消费进程或者生产线程和消费线程的工作能力来提高程序的整体处理数据的速度。*

```python
生产者负责生产数据，放到一个地方。消费者从这个地方拿数据。
这两者不挨着，两个进程互不打扰，也不等着对方，实现互相通信。
比如吃回转寿司，厨师做好食物放到传送带上，消费者从传送带上拿。
两者通过一个中间机制去通信。中间这个通信介质就可以用队列。厨师（生产者）进程负责不断把数据放到队列中，消费者进程可以不断从队列中获取数据。

由于队列底层是基于管道+锁实现的，已经实现了一种加锁的机制，保证多个进程往队列里放数据、取数据的时候不会乱。
队列的概念，rabitmq，使用消息队列机制，可以实现分布式（多个程序放到不同的机器上，提高计算资源、各种方面的硬件性能，分散压力。虽然分散了，但大家还是干同一个活，还是需要沟通交流，这就需要用到队列）。
```
...
进程池创建numprocess个进程，共享这几个进程。没必要再重新创建进程，节省创建进程的开销。
...
#p.apply(func [, args [, kwargs]]):
    ...
    apply提交一次，建一个进程，最多建 numprocess个。超出numprocess的，就需要等前面的进程结束一个进去一个。同步地提交任务，这个任务不执行完，下个任务不会执行，即串行。

#p.apply_async(func [, args [, kwargs]]):
    在一个池工作进程中执行func(*args,**kwargs),然后返回结果。此方法的结果是AsyncResult类的实例，callback是可调用对象，接收输入参数。当func的结果变为可用时，将立即传递给callback。callback禁止执行任何阻塞操作，否则将接收其他异步操作中的结果。
    把所有要执行的func放到进程池中，每放一个都不用等待，全部放完，numprocess个进程并发运行。
#p.close():
...
![image-20190206054736843](chapter12-Multiprocess.assets/image-20190206054736843.png)

openssh为linux系统自带的基本服务，既有server端，也有client端的包，用于linux主机之间彼此互为客户端、服务端。主要功能是远程连接、远程执行命令、远程拷贝文件。



修改服务器的端口号：vim  etc/ssh/sshd_config

修改以后让配置生效：systemctl restart sshd

查看修改后的端口号：netstat -an | grep 2222 或者 netstat -tln | grep 2222